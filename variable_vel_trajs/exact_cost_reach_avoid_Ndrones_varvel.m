function [negative_rob,xx,yy,zz] = exact_cost_reach_avoid_Ndrones_varvel(var,optParams)
...
C = 20; %const for smooth min/max operation %for 2 drones, use 10 with a period of 5s, casadi is unstable numerically
C1 = 20; %const for smooth max
...
for d = 1:optParams.N_drones
    ...
    rho_unsafe(d) = min(-temp);
    if(isnan(rho_unsafe(d)))
    keyboard;
    end
...
%     dz(:,p) = zz(:,combos(p,1))-zz(:,combos(p,2));
    ...
    %dists(p) = SmoothMin(mutual_distances,C);

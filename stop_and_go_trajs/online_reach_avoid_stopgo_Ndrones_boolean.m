...
opts.expand = false;
%add feas of robustness
g = [g;-cost_reach_avoid_Ndrones(w,optParams)];
lbg = [lbg;eps];
ubg = [ubg;inf];
prob = struct('f',0, 'x', w, 'g', g);
...
%% online part
state_now = zeros(3,optParams.N_drones);
drone_name{1} = 'crazym04';
%% ros init stfuf
if(0)
    rosinit;
    for d = 1:optParams.N_drones
    goal_pub{d} = rospublisher(strcat(drone_name{d},'/trackers_manager/line_tracker_eth1/goal'));
    msg_pub{d} = rosmessage(goal_pub{d});
    msg_pub{d}.x = 0;
    msg_pub{d}.y = 0;
    msg_pub{d}.z = 1;
    msg_pub{d}.relative = false;
    send(goal_pub{d},msg_pub{d});
    
    state_sub{d} = rossubscriber(strcat(drone_name{d},'/odom'),'nav_msgs/Odometry');
    msg_sub{d} = receive(state_sub{d});
    end
end
%%
%get init states for init guess
%
k = 0;
for d = 1:optParams.N_drones
state_now(:,d) = [msg_sub{d}.Pose.Pose.Position.X;...
    msg_sub{d}.Pose.Pose.Position.Y;...
    msg_sub{d}.Pose.Pose.Position.Z];

w((k)*3+1+(d-1)*Clen) = state_now(1,d); %x
w((k)*3+2+(d-1)*Clen) = state_now(2,d); %y
w((k)*3+3+(d-1)*Clen) = state_now(3,d); %z
lbw((k)*3+1+(d-1)*Clen) = state_now(1,d); %x
lbw((k)*3+2+(d-1)*Clen) = state_now(2,d); %y
lbw((k)*3+3+(d-1)*Clen) = state_now(3,d); %z
ubw((k)*3+1+(d-1)*Clen) = state_now(1,d); %x
ubw((k)*3+2+(d-1)*Clen) = state_now(2,d); %y
ubw((k)*3+3+(d-1)*Clen) = state_now(3,d); %z
end
%%
disp('Solving init...');
tic;
...
for i = 0:H_formula-1
    ...
    %update state and bounds on waypoints
    w((i)*3+1+(d-1)*Clen) = state_now(1,d); %x
    w((i)*3+2+(d-1)*Clen) = state_now(2,d); %y
    w((i)*3+3+(d-1)*Clen) = state_now(3,d); %z
    lbw((i)*3+1+(d-1)*Clen) = state_now(1,d); %x
    lbw((i)*3+2+(d-1)*Clen) = state_now(2,d); %y
    lbw((i)*3+3+(d-1)*Clen) = state_now(3,d); %z
    ubw((i)*3+1+(d-1)*Clen) = state_now(1,d); %x
    ubw((i)*3+2+(d-1)*Clen) = state_now(2,d); %y
    ubw((i)*3+3+(d-1)*Clen) = state_now(3,d); %z

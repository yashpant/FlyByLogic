function [w_opt, optParams, time_taken] = planMission_stop_and_go(missionHandle)
import casadi.*
...
K2_tprime = (120)*(t_prime^3)/(T^5) - (180)*(t_prime^2)/(T^4) + ...
    (60)*(t_prime)/(T^3);
...
prob = struct('f', Mission_Robustness_stop_and_go(var,optParams), 'x', var, 'g', g);
...
%         optParams,mission_type);
    [temp] = Mission_Get_Initial_Waypoints_stop_and_go(p0(:,i),...
        optParams,mission_type);
...
var0 = w0;
optParams.var0 = var0;
Init_waypoints_rob = Mission_Robustness_stop_and_go(var0,optParams);
toc
%% solve the nlp
disp('Solving...');
tic;
